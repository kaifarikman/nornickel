...
Без `VITE_API_MODE` клиент работает от `src/mocks/` для автономной демо-разработки.
В `VITE_API_MODE=http` фронт ходит в backend и не подменяет ошибки fixture-данными.
